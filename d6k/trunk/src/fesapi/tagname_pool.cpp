...
/*! \fn bool CTagNamePool::Initialize(const char* pszProjName, int32u nMode)
********************************************************************************************************* 
...
** \note 
********************************************************************************************************/
bool CTagNamePool::Initialize(const char* pszProjName, int32u nMode)
{
	Q_ASSERT(pszProjName);
	if (pszProjName == nullptr)
		return false;
	Q_ASSERT(strlen(pszProjName));
	if (strlen(pszProjName) <= 0)
		return false;
	...
	char * pData = (char*)m_pMem->GetShareMem("TagNamePool");

	Q_ASSERT(pData);
	if (pData == nullptr)
	{
		szLog = QString(QObject::tr("Open share memory [ %1 ] failed")).arg(pszProjName);
	...
	m_pMagicMem = reinterpret_cast<HEAD_MEM*>(pData);
	...
	m_nSystemVarCount = m_pMagicMem->SystemVariableCount;
	...
	// 创建节点表
	nRet = CreateTagNamePool(pHead, IDD_NODE, m_pMagicMem->NodeCount, MAX_NODE_OCCNO);
	pHead += nRet;
	...
	//创建AIN_ALARM
	nRet = CreateTagNamePool(pHead, IDD_AINALARM, m_pMagicMem->AinAlarmCount, MAX_ALARM_OCCNO);
	pHead += nRet;

	//创建AIN_ALARM_LIMIT
	nRet = CreateTagNamePool(pHead, IDD_AINALARMLIMIT, m_pMagicMem->AinAlarmLimitCount, MAX_ALARM_OCCNO);
	pHead += nRet;

	//创建DIN_ALARM
	nRet = CreateTagNamePool(pHead, IDD_DINALARM, m_pMagicMem->DinAlarmCount, MAX_ALARM_OCCNO);
	pHead += nRet;

	//创建DIN_ALARM_LIMIT
	nRet = CreateTagNamePool(pHead, IDD_DINALARMLIMIT, m_pMagicMem->DinAlarmLimitCount, MAX_ALARM_OCCNO);
	pHead += nRet;
	...
	//创建DOUT
	nRet = CreateTagNamePool(pHead, IDD_DOUT, m_pMagicMem->AoutCount, MAX_OCCNO);
	pHead += nRet;

	//创建AIN
	nRet = CreateTagNamePool(pHead, IDD_AIN, m_pMagicMem->AoutCount, MAX_OCCNO);
	pHead += nRet;

	//创建DIN
	nRet = CreateTagNamePool(pHead, IDD_DIN, m_pMagicMem->AoutCount, MAX_OCCNO);
	pHead += nRet;
