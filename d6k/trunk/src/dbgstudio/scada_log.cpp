...
CScadaLog::CScadaLog(const std::string & szModuleName, const char * szLog, int nLevel, size_t nIndex)
{
...
CScadaLog::CScadaLog(const std::string & szModuleName, const char * szLog, int nLevel, size_t nIndex, const char * szFileName, int nLine, const char * szFunc)
{
	m_szModuleName = szModuleName;
	m_szLogText = szLog;
	m_nLevel = nLevel;
...
CScadaLog::CScadaLog(CScadaLog *pLog)
{
	Q_ASSERT(pLog);
	Q_ASSERT(pLog != this);
	if (pLog && pLog != this)
...
CScadaLog& CScadaLog::operator= (const CScadaLog & other)
{
	if (this == &other)
	{
		return *this;
...
/*! \fn size_t CScadaLog::SaveToBuffer(unsigned char *pInBuff, size_t nInBuffLen) const
*********************************************************************************************************
...
** \note
********************************************************************************************************/
size_t CScadaLog::SaveToBuffer(unsigned char *pInBuff, size_t nInBuffLen) const
{
	Q_ASSERT(pInBuff);
	if (pInBuff == nullptr)
		return 0;
...
bool  CScadaLog::LoadFromBuffer(const char *pInBuff, size_t nInBuffLen)
{
	size_t nOffset = 0;
	size_t nSize = 0;
...
/*! \fn size_t CScadaLog::GetDataSize() const
*********************************************************************************************************
** \brief CScadaLog::GetDataSize
...
** \note
********************************************************************************************************/
size_t CScadaLog::GetDataSize() const
{
	size_t nOffset = 0;
	size_t nSize = 0;
...
void CScadaLog::SaveToLogData(LOG_DATA  *pData) const
{
	Q_ASSERT(pData);
	if (pData == nullptr)
		return;
...
bool CScadaLog::LoadFromLogData(const LOG_DATA *pData)
{
