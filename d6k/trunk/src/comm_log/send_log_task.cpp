...
/*! \fn void  CSendLogTask::AddLogMsg(CCommMsgLog *pMsg)
********************************************************************************************************* 
...
** \note 
********************************************************************************************************/
void  CSendLogTask::AddLogMsg(CCommMsgLog *pMsg)
{
	Q_ASSERT(pMsg);
	if (pMsg == nullptr)
		return;
...
bool CSendLogTask::GetLogString(CCommMsgLog *pMsg)
{
	Q_ASSERT(pMsg);
	if (pMsg == nullptr)
		return false;
...
bool  CSendLogTask::TakeMsgLog(std::shared_ptr<CCommMsgLog>&pLog)
{
	std::lock_guard<std::mutex> lock(m_LogLock);
	if (m_arrLogs.empty() == true)
		return false;
