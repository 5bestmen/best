...
//	OpenIO(szProjName.toStdString().c_str(),1);

	LogMsg("scd_alarm", QObject::tr("alarmview::open fesDB succeed ").toStdString().c_str(), 0, nullptr);

	OpenPostOffice("SCADA");
	int nMailID = 0;
	OpenMailBox("SCADA", "ALARM_SCD_WIN", &nMailID);
	//实现邮件订阅
	SubscribeMail("SCADA", "ALARM_SCD_WIN", MSG_EVT_ANA| MSG_EVT_DIGI| MSG_EVT_DIAG| MSG_EVT_OPER| MSG_EVT_SETVAL | MSG_EVT6| MSG_EVT7);

	LogMsg("scd_alarm", QObject::tr("alarmview::open fes mail system OK").toStdString().c_str(), 0, nullptr);
...
CAlarmWindow::~CAlarmWindow()
{
	...
	LogMsg("scd_alarm", QObject::tr("alarmview::module stopped").toStdString().c_str(), 0, nullptr);
}
