#include "predictaitable.h"

CPredictAiTable::CPredictAiTable(QWidget* parent, std::vector <CAiInput *> &vecAi, IMainModuleInterface* pCore)
	:QTableView(parent),m_pCore(pCore)
{
	...
	m_pModel = new CPredictAiModel(this, vecAi, pCore);
...
CPredictAiTable::~CPredictAiTable()
{

}

void CPredictAiTable::DoubleClicked(const QModelIndex& index)
{
...
void CPredictAiTable::ShowMouseRightButton(const QPoint& point)
{
...
void CPredictAiTable::AddAnalogPoint(QModelIndex& index)
{
	int nRow = index.row();
...
void CPredictAiTable::DeleteAnalogPoint(QModelIndex& index)
{
	int nRow = index.row();
