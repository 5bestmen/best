...
/*! \fn bool CServerDB::Initialize(RUN_MODE mode)
********************************************************************************************************* 
** \brief CServerDB::Initialize 
** \details 初始化
** \return bool 
** \author LiJin 
** \date 2016年9月1日 
...
size_t CServerDB::BuildDB(char * pAddr)
{
	Q_ASSERT(pAddr);
...
size_t CServerDB::BuildTagnamedDB(char* pAddr)
{
	Q_ASSERT(pAddr);
...
size_t CServerDB::BuildTagNameDB(char* pAddress, std::vector<TAG_OCCNO_DEF>& vec)
{
	Q_ASSERT(pAddress);
...
bool CServerDB::ReadScdVarFile(const char* pszPath)
{
	Q_ASSERT(pszPath);
...
void CServerDB::ReadNonLinearInfo()
{
...
void CServerDB::ReadNonLinearVal(TRANSFORM_NONLINEAR_DEF pTransFormNonLinear)
{
...
void CServerDB::ReadAinAlarmInfo()
{
...
void CServerDB::ReadDinAlarmInfo()
{
...
void CServerDB::ReadAinOfflimitInfo(AIN_ALARM_DEF pAinAlarm)
{
...
void CServerDB::ReadDinOfflimitInfo(DIN_ALARM_DEF pDinAlarm)
{
...
void CServerDB::SkipUnknownElement()
{
	m_XmlReader.readNext();
	while (!m_XmlReader.atEnd())
	{
...
void CServerDB::ReadSystemVariable()
{
	m_XmlReader.readNext();
...
void CServerDB::ReadUserVariable()
{
	m_XmlReader.readNext();
...
size_t CServerDB::BuildAlarmDB(char* pAddress)
{
	return 0;
}

size_t CServerDB::BuildAinAlarmDB(char* pAddress)
{
	Q_ASSERT(pAddress);
...
size_t CServerDB::BuildDinAlarmDB(char* pAddress)
{
	Q_ASSERT(pAddress);
...
size_t CServerDB::BuildLinearDB(char* pAddress)
{
	Q_ASSERT(pAddress);
...
size_t CServerDB::BuildNonLinearDB(char* pAddress)
{
	Q_ASSERT(pAddress);
...
size_t CServerDB::BuildSystemVariableDB(char* pAddress)
{
	Q_ASSERT(pAddress);
...
size_t CServerDB::BuildUserVariableDB(char* pAddress)
{
	Q_ASSERT(pAddress);
