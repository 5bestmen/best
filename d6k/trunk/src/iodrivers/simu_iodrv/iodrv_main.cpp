...
extern "C"
{
	...
	********************************************************************************************************/
	IODRV int StartIoDriver(const char * pszName, int32u nMyNodeOccNo, int32u nChannelOccNo)
	{
		...
		{
			// TODO : LOG
			LogMsg("simu_iodrv",QObject::tr("Open fes kernel failed...").toStdString().c_str() ,2,nullptr);
			return 0;
		}
		// LOG 成功
		LogMsg("simu_iodrv", QObject::tr("Open fes kernel successed...").toStdString().c_str(), 0,nullptr);
	...
	{		
		...
		{
			// TODO : LOG
			LogMsg("simu_iodrv", QObject::tr("Close fes kernel failed...").toStdString().c_str(), 2,nullptr);
			return 0;
		}
