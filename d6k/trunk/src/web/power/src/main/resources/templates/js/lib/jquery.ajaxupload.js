...
(function ($) {
	...
	$.ajaxUploadSerializeFiles = function( element ,other) {
		var data = [];
		...
		data.push({'name':other.attr("name"),'value':other.val()})
		return data;
	};
	...
	$.ajaxUploadPrompt = function( options ) {
		...
		var other = $('<input type="text" />').appendTo(form);
		other.attr('name',$.ajaxUploadSettings.othername);
		other.attr('value',$.ajaxUploadSettings.othervalue);
		...
		d.change(function() {
			...
			s.files = this.files;
			s.data = $.ajaxUploadExtractData(s.data, $.ajaxUploadSerializeFiles(this,other));
