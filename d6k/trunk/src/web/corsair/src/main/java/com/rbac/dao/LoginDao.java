...
public class LoginDao extends BaseDaoSupport {
	...
	public SysAccount getSysAccountByUsername(String username) {
		List list = super.currentSession().createCriteria(SysAccount.class).add(
	...
	public List<SysMenu> getMenuListByAccountId(Long accountId){
		...
		return super.currentSession().createQuery(hql).setLong("accountId", accountId).list();
	...
	public List<SysAction> getActionListByAccountId(Long accountId){
		...
		return super.currentSession().createSQLQuery(sql).addEntity(SysAction.class).setLong("accountId", accountId).list();
	}
}
