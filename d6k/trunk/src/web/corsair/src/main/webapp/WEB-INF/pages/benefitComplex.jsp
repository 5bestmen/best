...
<body>
  ...
  <%--<div id="wrapper">
    ...
    <div id="page-wrapper">--%>
      <div class="row">
        <div class="col-lg-12">
          <h2 class="page-header">综合效益</h2>
        </div>
      </div>
      <div class="row">
        <div style="text-align:left; padding-bottom:5px; width: 80%">
          <form:form cssClass="form-horizontal" action="benefitComplex" method="post" commandName="benefitComplexForm">
            ...
            <div class="table-responsive">
              <table id="targetTable" class="table table-bordered table-striped" style="text-align: center">
                <tr style="text-align: center">
                  <td colspan="6" class="gray-bg"><h4><strong>${initDate}${stationName}综合效益结算单</strong></h4></td>
                </tr>
                <tr>
                  <td>抄表日期</td>
                ...
                <tr style="text-align: center">
                  <td colspan="6" class="gray-bg"><h4><strong>高配反向上网电量总计量</strong></h4></td>
                </tr>
                <tr>
                ...
                <tr style="text-align: center">
                  <td colspan="6" class="gray-bg"><h4><strong>光伏电站总发电量</strong></h4></td>
                </tr>
                <tr>
                  <td class="table-head"></td>
                  <td>电表编码</td>
                ...
                <c:forEach items="${totalElectricList}" var="ElectricPSR" varStatus="j">
                  <tr>
                    <c:if test="${j.count == 1}">
                      <td rowspan="${fn:length(totalElectricList)}" style="vertical-align:middle" class="Electric-color">并网柜计量表</td>
                    </c:if>
                    <td>${ElectricPSR.getName()}</td>
                    <td>${ElectricPSR.getCurValue()}</td>
                    <td>${ElectricPSR.getLastValue()}</td>
                ...
                <tr style="text-align: center">
                  <td colspan="6" class="gray-bg"><h4><strong>公司自用电费结算</strong></h4></td>
                </tr>
                <tr>
                  <td rowspan="${2 + fn:length(totalElectricList)}" style="vertical-align:middle">自用电量结算</td>
                  <td>电表编码</td>
                  <td>电量（kWh）</td>
                  <td colspan="2">结算电价（元/kWh）</td>
                  <td>电费（元）</td>
                </tr>
                <c:forEach items="${totalElectricList}" var="ElectricPSR" varStatus="m">
                  <tr>
                    <td>${ElectricPSR.getName()}</td>
                    <td>${ElectricPSR.getElectricValue()}</td>
                    <td colspan="2">${ElectricPSR.getElectricPrice()}
                      <c:if test="${m.count == 1}">
                        （燃油加工费）
                      </c:if>
                    </td>
                ...
                <tr>
                  ...
                  <td colspan="4" ><strong>${totalCompanyFee}</strong></td>
                </tr>
                <tr style="text-align: center">
                ...
                <c:forEach items="${netElectricAccount}" var="ElectricPSR" varStatus="i">
                  <tr>
                    <c:if test="${i.count == 1}">
                      <td rowspan="${fn:length(netElectricAccount)+1}" style="vertical-align:middle">上网电量结算</td>
                    </c:if>
                    <td>${ElectricPSR.getName()}</td>
                    <td>${ElectricPSR.getElectricValue()}</td>
                ...
                <tr style="text-align: center">
                  <td colspan="6" class="gray-bg"><h4><strong>各级补贴（元/kWh）</strong></h4></td>
                </tr>
                <tr>
                  <td>国家补贴</td>
                  <td>${StateSubsidyPrice}</td>
                  <td>省补贴</td>
                  <td>${provinceSubsidyPrice}</td>
                  <td>市补贴</td>
                  <td>${citySubsidyPrice}</td>
                </tr>
                <tr>
                  <td>县补贴</td>
                  <td>${countySubsidyPrice}</td>
                  <td colspan="4"></td>
                </tr>
                <tr>
                  <td colspan="2">总发电量(kWh）</td>
                  <td>国家补贴(元)</td>
                  <td>省补贴(元)</td>
                  <td>市补贴(元)</td>
                  <td>县补贴(元)</td>
                </tr>
                <c:forEach items="${subsidyAccountList}" var="ElectricPSR" varStatus="n">
                  <tr>
                    <td>${ElectricPSR.getName()}</td>
                    <td>${ElectricPSR.getElectricValue()}</td>
                    <td>${ElectricPSR.getElectricPrice()}</td>
                    <td>${ElectricPSR.getCurValue()}</td>
                    <td>${ElectricPSR.getLastValue()}</td>
                    <td>${ElectricPSR.getFee()}</td>
                  </tr>
                ...
                <tr>
                  <td colspan="2"><strong>总计补贴金额</strong></td>
                  <td colspan="4"><strong>${totalSubsidyFee}</strong></td>
                </tr>
                <tr style="text-align: center">
                  <td colspan="6" class="gray-bg"><h4><strong>合计结算金额（元）</strong></h4></td>
                </tr>
                <tr>
                  <td rowspan="3"  style="vertical-align:middle">总结算金额</td>
                  <td>自用电费金额</td>
                  <td>上网总电价</td>
                  <td>补贴总金额</td>
                  <td><strong>合计收益</strong></td>
                  <td></td>
                </tr>
                <tr>
                  <td>${totalCompanyFee}</td>
                  <td>${totalNetElectricFee}</td>
                  <td>${totalSubsidyFee}</td>
