...
CForwarding_select_ao_tagname_dialog::CForwarding_select_ao_tagname_dialog(QWidget *parent, Config::CFesData *pFes)
	: QDialog(parent), m_pModel(nullptr), m_strTagName("")
{
	ui.setupUi(this);
...
CForwarding_select_ao_tagname_dialog::~CForwarding_select_ao_tagname_dialog()
{

}

void CForwarding_select_ao_tagname_dialog::InitChannel(CQuoteItem *pItem, Config::CFesData *pFesData)
{
	Q_ASSERT(pItem);
	if (!pItem)
	{
...
bool CForwarding_select_ao_tagname_dialog::InitChannelGroup(CQuoteItem *pItem, Config::CChannelGroup *pGroup)
{
	QString strTmp;
	CQuoteItem *newItem;
...
void CForwarding_select_ao_tagname_dialog::InitDevice(CQuoteItem *pItem, std::vector <Config::CDevice *> &pTmp)
{
	Q_ASSERT(pItem);
	if (!pItem)
	{
...
void CForwarding_select_ao_tagname_dialog::DoubleClicked(const QModelIndex &index)
{
	...
	if (nType == FES_DEVICE_AO)
	{
		...
		m_pModel = new CForwardingAOSelectDataModel(nullptr, pTmp);
		ui.tableView->setModel(m_pModel);
	}
}

void CForwarding_select_ao_tagname_dialog::OnOk()
{
	...
	QModelIndex index = model->index(row, CForwardingAOSelectDataModel::COLUMN::TagName);//
...
void CForwarding_select_ao_tagname_dialog::OnCancel()
{
	m_strTagName.clear();
