...
#include "forwarding_select_domodel.h"
...
CForwarding_select_do_tagname_dialog::CForwarding_select_do_tagname_dialog(QWidget *parent, Config::CFesData *pFes)
...
CForwarding_select_do_tagname_dialog::~CForwarding_select_do_tagname_dialog()
{
}

void CForwarding_select_do_tagname_dialog::InitChannel(CQuoteItem *pItem, Config::CFesData *pFesData)
{
	Q_ASSERT(pItem);
	if (!pItem)
	{
...
bool CForwarding_select_do_tagname_dialog::InitChannelGroup(CQuoteItem *pItem, Config::CChannelGroup *pGroup)
{
	QString strTmp;
	CQuoteItem *newItem;
...
void CForwarding_select_do_tagname_dialog::InitDevice(CQuoteItem *pItem, std::vector <Config::CDevice *> &pTmp)
{
	Q_ASSERT(pItem);
	if (!pItem)
	{
	...
	for (int32u i = 0; i < pTmp.size(); i++)
	{
		...
		//auto strDesc = QObject::tr("ao");
		//auto strAOTag = QString("%1_%2").arg(pTmp[i]->m_szTagName).arg(strDesc);
		//auto pSubItem = new CQuoteItem(strAOTag);
		//pSubItem->setData(strAOTag, LASTVALUE);
		...
		auto strDesc = QObject::tr("do");
		auto strDOTag = QString("%1_%2").arg(pTmp[i]->m_szTagName).arg(strDesc);
		auto pSubItem = new CQuoteItem(strDOTag);
		pSubItem->setData(strDOTag, LASTVALUE);
...
void CForwarding_select_do_tagname_dialog::DoubleClicked(const QModelIndex &index)
{
	...
	if (nType == FES_DEVICE_DO)
	{
		...
		m_pModel = new CForwardingDOSelectDataModel(nullptr, pTmp);
		ui.tableView->setModel(m_pModel);
	}
}

void CForwarding_select_do_tagname_dialog::OnOk()
{
	...
	QModelIndex index = model->index(row, CForwardingDOSelectDataModel::COLUMN::TagName);//
...
void CForwarding_select_do_tagname_dialog::OnCancel()
{
	m_strTagName.clear();
