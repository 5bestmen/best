...
CSelectAlarmDialog::CSelectAlarmDialog(QWidget *parent, Config::CFesData *pFes, void *pData, int nType)
	: QDialog(parent), m_pModel(nullptr)
{
	...
	if (nType == AIALARM)
	{
		auto pTmp = static_cast<Config::CAnalogInput *>(pData);
	...
	{
		auto pTmp = static_cast<Config::CDigitalInput *>(pData);
		m_nAlarmType = pTmp->m_nAlarmType;
		m_strDesc = pTmp->m_strAlarmDesc;
		m_strTagName = pTmp->m_strAlarmTag;

		DumpModel(m_pModel, m_pModel->indexFromItem(m_pModel->invisibleRootItem()), pTmp->m_strAlarmTag);
	}
...
CSelectAlarmDialog::~CSelectAlarmDialog()
{
...
** \note 
********************************************************************************************************/
void CSelectAlarmDialog::InitAlarm(int nType)
{
	//报警
...
bool CSelectAlarmDialog::InitAIAlarmGroup(CQuoteItem *pItem, Config::CAnalogAlarmGroup *pGroup)
{
	Q_ASSERT(pItem);
	if (!pItem)
	{
...
bool CSelectAlarmDialog::InitDIAlarmGroup(CQuoteItem *pItem, Config::CDigitalAlarmGroup *pGroup)
{
	Q_ASSERT(pItem);
	if (!pItem)
	{
...
** \note 
********************************************************************************************************/
void CSelectAlarmDialog::InitAIOfflimit(CQuoteItem *pItem, std::vector<Config::CAinAlarmLimit *> &m_arrAlarmOfflimit)
{
	Q_ASSERT(pItem);
...
** \note 
********************************************************************************************************/
void CSelectAlarmDialog::InitDIAlarm(CQuoteItem *pItem, QMap<QString, CQuoteItem *> &pGroup, Config::CFesData *pFesData)
{
	Q_ASSERT(pItem);
...
** \note 
********************************************************************************************************/
void CSelectAlarmDialog::InitDIOfflimit(CQuoteItem *pItem, std::vector<Config::CDinAlarmLimit *> &m_arrOfflimit)
{
	Q_ASSERT(pItem);
...
** \note 
********************************************************************************************************/
void CSelectAlarmDialog::InitDIAlarm(Config::CDigitalAlarm *pDigtalAlarm)
{
...
** \note 
********************************************************************************************************/
void CSelectAlarmDialog::Clicked(const QModelIndex &index)
{
...
** \note 
********************************************************************************************************/
void CSelectAlarmDialog::DumpModel(const QAbstractItemModel *pModel, const QModelIndex & top, const QString &s)
{
	QModelIndex Item;
...
void CSelectAlarmDialog::OnCancel()
{
	rejected();
...
void CSelectAlarmDialog::showMouseRightButton(const QPoint& point)
{
...
void CSelectAlarmDialog::InitPara()
{
	QStringList list;
	list.clear();
	list << tr("TagName") << tr("Description") << tr("Enable") << tr("OnQualityGood") << tr("DeadType") << tr("DeadArea")
