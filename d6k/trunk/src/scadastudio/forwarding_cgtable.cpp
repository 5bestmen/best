...
CForwardingCGTable::CForwardingCGTable(QWidget *parent, IMainModuleInterface *pCore, Config::CForwardingChannelData *pChannelData, Config::CFesData *pFes)
	: QTableView(parent), m_pFes(nullptr)
{
	...
	m_pModel = new CForwardingCGModel(this, pCore, pChannelData, pFes);
...
CForwardingCGTable::~CForwardingCGTable()
{

}

void CForwardingCGTable::refresh()
{
	m_pModel->RefrushModel();
}

void CForwardingCGTable::mouseReleaseEvent(QMouseEvent * event)
{
	//QSet<int32s> set;
...
** \note 
********************************************************************************************************/
void CForwardingCGTable::ShowMouseRightButton(const QPoint& point)
{
...
** \note 
********************************************************************************************************/
void CForwardingCGTable::AddAnalogPoint(QModelIndex &index)
{
	int nRow = index.row();

	auto arr = m_pModel->GetCGs();
	nRow = (int)arr.size();
...
** \note 
********************************************************************************************************/
void CForwardingCGTable::DeleteAnalogPoint(QModelIndex &index)
{
	if (!index.isValid())
	{
		return;
...
void CForwardingCGTable::DoubleClicked(const QModelIndex & index)
{
