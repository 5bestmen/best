...
//please note that even if you defined ASIO_REUSE_OBJECT macro, ST_ASIO_FREE_OBJECT_INTERVAL macro is still useful, it will make st_object_pool
...
#define ST_ASIO_FREE_OBJECT_INTERVAL	10 //seconds, validate only if ASIO_REUSE_OBJECT not defined
#endif
#endif
...
namespace ASIO
{
	...
	protected:
		...
		struct invalid_object
		{
			...
			bool IsTimeout(time_t now) const
			{
				return kick_out_time <= now - ST_ASIO_OBSOLETED_OBJECT_LIFE_TIME;
			}
#endif
		};
	...
	protected:
		...
		{
#ifndef ST_ASIO_FREE_OBJECT_INTERVAL
			SetTimer(TIMER_FREE_SOCKET, 1000 * ST_ASIO_FREE_OBJECT_INTERVAL, [this](unsigned char id)->bool {assert(TIMER_FREE_SOCKET == id);  free_object(); return true; });
#endif
