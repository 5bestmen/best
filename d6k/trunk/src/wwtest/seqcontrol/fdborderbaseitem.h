...
public:
    ...
    //
    QList<LOGICITEMPRO> GetAllInputPro();
    QList<LOGICITEMPRO> GetOutputPro();
    //设置输入端子
...
public:
    ...
    virtual int type() const{ return OBJECTITEM; }
    //获取连线的状态
    int GetItemIOStatus();
    //设置执行顺序编号
    ...
    void CloseEnoItems();
    //增加eno信息
    void AddEnoItems(QPainter *painter);
    //获取注释信息
...
private:
    ...
    int m_nOutMaxNum;
    //输入端信息  端子->属性信息
    QMap<int,LOGICITEMPRO> m_mapInputPro;
    //输出端信息 端子->属性信息
    QMap<int,LOGICITEMPRO> m_mapOutPutPro;
    //输入列表的名称
