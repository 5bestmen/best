...
** \note 
********************************************************************************************************/
CFesModel::CFesModel(CCoreui *pUi, CQuoteItem *item)
{
	//Test
	...
	//CChannel *pChannel = new CChannel(pUi);
	...
	//前置节点
	m_pTopFesItem = new CQuoteItem(QIcon(FES_PNG), QStringLiteral("前置组态"));
...
void CFesModel::AddChannelGroup(QModelIndex &index, int nType)
{
	...
	if(item->data(Qt::WhatsThisRole).toInt() < MAX_GROUP_NUM)
	{
		...
		newItem->setData(FES_CHANNEL_GROUP_ITEM, TAGNAME);
...
void CFesModel::doubleClicked(const QModelIndex &index)
{
...
void CFesModel::showMouseRightButton(const QPoint &point)
{
