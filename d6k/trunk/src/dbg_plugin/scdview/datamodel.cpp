...
QVariant CDataModel::ShowDoutData(int nRow, int nColumn) const
{
	...
	case DOUT_ScanEna:
		return QString::number(pDout->ScanEna);
	case DOUT_Init:
...
bool CDataModel::GetTotalInfo()
{
	...
	case AIN_TYPE:
	{
		...
		size_t nAinCount = m_pMem->GetChannelMgr()->GetDevice(m_nCurrentDeviceOccNo-1)->GetAinCount();

		for (int i = 0; i < nAinCount;++i)
		{
			AIN * pAin;

			m_pMem->GetAINByOccNo(i+1,&pAin);
	...
	case DIN_TYPE:
	{
		size_t nDinCount = m_pMem->GetChannelMgr()->GetDevice(m_nCurrentDeviceOccNo - 1)->GetDinCount();

		for (int i = 0; i < nDinCount; ++i)
		{
			DIN * pDin;

			m_pMem->GetDINByOccNo(i + 1, &pDin);
