...
public:
	...
	//赋值函数
	const CGraphicsLayer& operator=(const CGraphicsLayer &src);
public:
...
public:
	...
	std::size_t GetWidgetCount()
	{
		return m_arrWidgets.size();
	}

	CBaseWidget *GetFirstSelectedWidget()
	{
    ...
    //循转
    bool RotatingWidget(const QPointF & delta);

	bool GroupWidgets(CGroupWidget *pGroup);
    //组合指定的图元  
	CGroupWidget * GroupInputWidgets(const std::vector <CBaseWidget *> &arr, double dAngle, GROUP_CHILD);

	//
	bool CroupGroupWidgets(const std::vector <CBaseWidget *> &arr, double dAngle, CGroupWidget *group);
	//
	bool CreateGroupWidgets(const std::vector <CBaseWidget *> &arr, double dAngle, CGroupWidget *pGroup);
	...
	CBaseWidget *HitTest(QPoint & pos);
